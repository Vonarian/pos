...
class ApiClient {
  ...
  Future<Map<String, dynamic>> getQuadrants(String date) async {
    final response = await _dio.get('/api/v1/routines/quadrants', queryParameters: {'date': date});
    return response.data as Map<String, dynamic>;
  }

  Future<void> completeRoutine(String id, {DateTime? completedAt}) async {
    await _dio.post('/api/v1/routines/complete', data: {
      'id': id,
  ...
  Future<Map<String, dynamic>> pullSync({DateTime? since}) async {
    ...
    final response = await _dio.get('/api/v1/sync/pull', queryParameters: params);
    return response.data as Map<String, dynamic>;
  }

  Future<Map<String, dynamic>> getDailySummary(String date) async {
    final response = await _dio.get('/api/v1/metrics/daily-summary', queryParameters: {'date': date});
    return response.data as Map<String, dynamic>;
  }
}
