...
class OfflineMetricRepository {
  ...
  OfflineMetricRepository({
    required this.db,
    this.apiClient,
  });

  Future<void> ingestMetrics(List<HealthDataPoint> points) async {
    final companions = points.map((p) => HealthMetricsTableCompanion.insert(
      id: p.id,
      source: p.source,
    ...
    if (apiClient != null) {
      try {
        ...
        return {
          ...
          'CALORIES_BURNED': (remote['calories_burned'] as num?)?.toDouble() ?? 0.0,
          'SLEEP_DURATION': (remote['sleep_minutes'] as num?)?.toDouble() ?? 0.0,
